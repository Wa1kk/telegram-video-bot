...
// ---------------------------------------------------------------------------
// BotGuard environment (jsdom)
// ---------------------------------------------------------------------------

/**
 * Temporarily install jsdom globals (window, document) on globalThis so that
 * BotGuard's script can run.  Returns { cleanup }.
 */
function installBotGuardEnvironment() {
  ...
  if (!w.getComputedStyle) w.getComputedStyle = () => ({});

  const propsToSet = { window: w, document: w.document };
  const saved = {};

...
// Public API keys used by YouTube's own web/mobile players.
const INNERTUBE_KEYS = {
  WEB: 'AIzaSyAO_FJ2SlqU8Q4STEHLGCilw_Yd_0C0s8M',
  MWEB: 'AIzaSyAO_FJ2SlqU8Q4STEHLGCilw_Yd_0C0s8M',
  ANDROID: 'AIzaSyA8eiZmM1FaDVjRy-jfRyTgkS1m0IhZZEo',
};
...
const INNERTUBE_CLIENTS = [
  ...
  {
    name: 'MWEB',
    key: INNERTUBE_KEYS.MWEB,
    context: {
      client: {
        clientName: 'MWEB',
        clientVersion: '2.20241117.00.00',
        hl: 'ru',
        gl: 'RU',
      },
    },
...
async function tryInnertube(videoId, clientConfig) {
  const url = `https://www.youtube.com/youtubei/v1/player?key=${clientConfig.key}&prettyPrint=false`;

  const isAndroid = clientConfig.name === 'ANDROID';
  const headers = {
    'Content-Type': 'application/json',
  };

  if (isAndroid) {
    headers['User-Agent'] = 'com.google.android.youtube/19.29.37 (Linux; U; Android 11; ru) gzip';
  } else {
    headers['User-Agent'] = UA;
  ...
  // WEB/MWEB clients need playbackContext
  if (!isAndroid) {

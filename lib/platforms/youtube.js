...
function installBotGuardEnvironment() {
  ...
  window.crypto = globalThis.crypto || require('crypto').webcrypto;
  window.btoa = globalThis.btoa || ((s) => Buffer.from(s, 'binary').toString('base64'));
  window.atob = globalThis.atob || ((s) => Buffer.from(s, 'base64').toString('binary'));
  window.performance = globalThis.performance || { now: () => Date.now(), timeOrigin: Date.now() };
  window.screen = { width: 1920, height: 1080, availWidth: 1920, availHeight: 1040, colorDepth: 24 };
  window.innerWidth = 1920;
  window.innerHeight = 1080;
  window.devicePixelRatio = 1;
  window.history = { length: 1, pushState: () => {}, replaceState: () => {} };
  ...
  // BotGuard uses trustedTypes — provide a no-op policy
  ...
  const propsToSet = { window, document, navigator: window.navigator, location: loc };
  const saved = {};

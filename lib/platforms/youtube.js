...
// Piped instances — open-source YouTube frontends that handle auth server-side.
...
/**
 * Try downloading via a single Piped instance.
 * Piped is an open-source YouTube frontend that handles auth server-side.
 * Returns { stream, caption } or throws.
 */
async function tryPipedInstance(instanceUrl, videoId) {
  ...
  if (!res.ok) {
    throw new Error(`Piped HTTP ${res.status}`);
  }

  const data = await res.json();

  if (data.error) {
    throw new Error(`Piped: ${data.message || data.error}`);
  }

  // Find best video stream (mp4, <=720p)
  const videoStreams = (data.videoStreams || [])
    .filter(s => s.mimeType && s.mimeType.startsWith('video/mp4') && s.videoOnly === false)
    .sort((a, b) => (b.height || 0) - (a.height || 0));

  // Prefer <=720p
  let stream = videoStreams.find(s => s.height <= 720) || videoStreams[0];

  // If no combined stream, try video-only + audio
  if (!stream) {
    const videoOnly = (data.videoStreams || [])
      .filter(s => s.mimeType && s.mimeType.startsWith('video/mp4') && s.videoOnly === true)
      .sort((a, b) => (b.height || 0) - (a.height || 0));
    stream = videoOnly.find(s => s.height <= 720) || videoOnly[0];
  }

  if (!stream || !stream.url) {
    throw new Error('No suitable video stream found in Piped response');
  }

  // Build metadata from Piped response
  const meta = {
    title: data.title || '',
  ...
  const downloaded = await downloadToTmp(stream.url);

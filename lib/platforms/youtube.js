...
function installBotGuardEnvironment() {
  ...
  // Helper: safely set a property even if it has a read-only getter
  ...
  // BotGuard probes many browser APIs — stub everything it may touch
  const loc = {
  ...
  safeSet(window, 'location', loc);
  safeSet(window, 'origin', 'https://www.youtube.com');
  safeSet(window, 'navigator', nav);
  safeSet(window, 'matchMedia', () => ({ matches: false, addListener: () => {}, removeListener: () => {}, addEventListener: () => {}, removeEventListener: () => {} }));
  safeSet(window, 'requestAnimationFrame', (cb) => setTimeout(cb, 0));
  safeSet(window, 'cancelAnimationFrame', (id) => clearTimeout(id));
  safeSet(window, 'getComputedStyle', () => new Proxy({}, { get: () => '' }));
  safeSet(window, 'crypto', globalThis.crypto || require('crypto').webcrypto);
  safeSet(window, 'btoa', globalThis.btoa || ((s) => Buffer.from(s, 'binary').toString('base64')));
  safeSet(window, 'atob', globalThis.atob || ((s) => Buffer.from(s, 'base64').toString('binary')));
  safeSet(window, 'performance', globalThis.performance || { now: () => Date.now(), timeOrigin: Date.now() });
  safeSet(window, 'screen', { width: 1920, height: 1080, availWidth: 1920, availHeight: 1040, colorDepth: 24 });
  safeSet(window, 'innerWidth', 1920);
  safeSet(window, 'innerHeight', 1080);
  safeSet(window, 'devicePixelRatio', 1);
  safeSet(window, 'history', { length: 1, pushState: () => {}, replaceState: () => {} });
  safeSet(window, 'localStorage', { getItem: () => null, setItem: () => {}, removeItem: () => {} });
  safeSet(window, 'sessionStorage', { getItem: () => null, setItem: () => {}, removeItem: () => {} });
  safeSet(window, 'fetch', globalThis.fetch);
  safeSet(window, 'setTimeout', globalThis.setTimeout);
  safeSet(window, 'setInterval', globalThis.setInterval);
  safeSet(window, 'clearTimeout', globalThis.clearTimeout);
  safeSet(window, 'clearInterval', globalThis.clearInterval);
  if (!window.trustedTypes) {
    safeSet(window, 'trustedTypes', {
      createPolicy: () => ({ createHTML: (s) => s, createScript: (s) => s, createScriptURL: (s) => s }),
    });
  }

  const propsToSet = { window, document, navigator: nav, location: loc };
  const saved = {};
...
async function createPoTokenSession() {
  ...
  try {
    ...
    const rawVisitorData = innertube.session.context.client.visitorData;
    if (!rawVisitorData) throw new Error('Could not obtain visitorData');
    const visitorData = decodeURIComponent(rawVisitorData);
...
async function downloadWithColdStartToken(videoId) {
  ...
  let visitorData = yt.session.context.client.visitorData;
  if (!visitorData) throw new Error('No visitorData');

  // visitorData may be URL-encoded (%3D etc.) — decode it

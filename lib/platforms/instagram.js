 ...
 * Returns { csrfToken, cookieString }.
 */
async function getSession() {
  ...
  // Build cookie string: "csrftoken=xxx; mid=yyy; ig_did=zzz; ..."
  const cookieParts = rawCookies.map(c => c.split(';')[0]);
  const cookieString = cookieParts.join('; ');

  const csrfPart = cookieParts.find(c => c.startsWith('csrftoken='));
  if (!csrfPart) throw new Error('Could not get CSRF token from Instagram');
 ...
 * Retries up to 3 times on 429/401.
 */
async function fetchMediaData(shortcode, retries = 3) {
  ...
  if (!res.ok) {
    if ([401, 403, 429].includes(res.status) && retries > 0) {
      const wait = res.status === 429 ? 3000 : 1000;
      await new Promise(r => setTimeout(r, wait));
      return fetchMediaData(shortcode, retries - 1);
    }
    throw new Error(`Instagram GraphQL HTTP ${res.status}`);
  }

...
const SUPPORTED_URL_RE =
  /https?:\/\/(www\.)?(youtube\.com\/(shorts|watch)|youtu\.be\/|instagram\.com\/(p|reel|tv)\/|([a-z]+\.)?pinterest\.(com|ru|co\.\w+|fr|de|it|es|jp|ca|au|br|se|pt|nl|at|ch|be|dk|fi|no|pl)\/pin\/|pin\.it\/)[^\s]*/gi;
...
async function processUpdate(update) {
  ...
  try {
    ...
    const { stream, title, description } = await downloadVideo(url, platform);
    ...
    await sendVideo(chatId, stream, 'video.mp4', caption, messageId);

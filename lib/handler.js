...
// Matches YouTube Shorts, Instagram Reels/posts, Pinterest pins, TikTok
const SUPPORTED_URL_RE =
  /https?:\/\/(www\.)?(youtube\.com\/shorts|instagram\.com\/(p|reel|reels|tv|share\/r|share\/reel)\/|([a-z]+\.)?pinterest\.(com|ru|co\.\w+|fr|de|it|es|jp|ca|au|br|se|pt|nl|at|ch|be|dk|fi|no|pl)\/pin\/|pin\.it\/|tiktok\.com\/[^\s]+|vm\.tiktok\.com\/[^\s]+)[^\s]*/gi;
...
async function processUpdate(update) {
  ...
  // Check for non-Shorts YouTube links and notify the user
  const text = message.text || message.caption || '';
  const ytVideoRe = /https?:\/\/(www\.)?(youtube\.com\/watch|youtu\.be\/)/i;
  const ytVideoMatch = text.match(ytVideoRe);
  if (ytVideoMatch && urls.length === 0) {
    await sendMessage(chatId, '❌ Я скачиваю только YouTube Shorts. Обычные видео не поддерживаются.', messageId).catch(() => {});
    return;
  }
